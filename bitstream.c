...
struct bitstream *open_bitstream(const char *fichier, const char* mode)
{
    ...
    ALLOUER(stream, 1) ;
    stream->ecriture = mode[0] != 'r';
    stream->nb_bits_dans_buffer = 0;

    if (strcmp(fichier,"-") == 0) {
        if (stream->ecriture) {
            stream->fichier = stdout;
        } else {
            stream->fichier = stdin;
        }
    }
    else
        stream->fichier = fopen(fichier,mode);
    if (!stream->fichier)
    {
        free(stream);
...
{
    if (b->nb_bits_dans_buffer && b->ecriture) {
        if (fwrite(&(b->buffer), sizeof(Buffer_Bit), 1, b->fichier) != 1)
...
void put_bit(struct bitstream *b, Booleen bit)
{
    ...
    b->buffer = (Buffer_Bit) pose_bit(b->buffer, NB_BITS - 1 - b->nb_bits_dans_buffer, bit);
...
Booleen get_bit(struct bitstream *b)
{
    ...
    Booleen a = prend_bit(b->buffer, b->nb_bits_dans_buffer - 1);
    (b->nb_bits_dans_buffer)--;
    return a;
}
